...
class Attacks:
    ...
    def __init__(
        ...
        self.definitions = {
            ...
            for k in list(
                self.definitions.keys() if only_attacks == [] else only_attacks
            )
    ...
    def translate(self, name, max_search_depth=""):
        ...
        strings = {
            ...
            "depends": str(attack["Depends"] if "Depends" in attack else ""),
            "grants": str(attack["Grants"] if "Grants" in attack else ""),
        ...
        options = {
            ...
            "Admin": bool(
                True
                if "Grants" in attack and attack["Grants"] == "Admin"
                else False
            ),
            **dict(definition["Options"] if "Options" in definition else {}),
        }
        ...
        def cypher_resolve_commands(history=False):
            ...
            for placeholder, attr in sorted(
                ...
                if placeholder == attack["Affects"]:
                    substitute = (
                        "grant" if "Grants" not in attack else "option"
                    )

                elif "Depends" in attack and placeholder == attack["Depends"]:
                    substitute = "option"

                elif "Grants" in attack and placeholder == attack["Grants"]:
                    substitute = "grant"
                ...
                resolved = (
                    f'REPLACE({resolved}, "${{{placeholder}}}", {substitute})'
                )

            resolved = (
                ("[_ IN {}|{}]".format(strings["commands"], resolved))
                .replace("{", "{{")
                .replace("}", "}}")
            )
        ...
        def cypher_resolve_placeholder(placeholder):
            if placeholder == "":
                ...
                r = re.compile(r"(AWS::[A-Za-z0-9:]+)").match(placeholder)

                if r.group(1) is not None:
                    return str(
                        f"{r.group(1).replace(':', '').lower()}"
                        f":`{r.group(1)}`"
                    )
        ...
        def cypher_inject():
            ...
            for k, v in {
                k: cypher_resolve_placeholder(k)
                for k in {
                    r
                    for (r, _) in re.findall(
                        r"\$\{(AWS\:\:[A-Za-z0-9]+\:\:[A-Za-z0-9]+)?(\.[A-Za-z]+)?\}",
                        inject,
                    )
                }
            ...
            inject = re.sub(
                ...
                lambda x: "".join(
                    [
                        str(  # D (Direct)
                            f"[:TRANSITIVE|ATTACK*0..{strings['depth']}]->()-"
                            if x.groupdict()["directive"] == "I"
                            # I (Indirect)
                            else f"[:TRANSITIVE*0..{strings['depth']}]->()-"
                            if x.groupdict()["directive"] == "D"
                            # None
                            else ""
                        ),
                        f"[:{x.groupdict()['type']}{{",
                    ...
                    (
                        ...
                        + ", ".join(
                            [
                                ...
                                else "{i}[0] AS {i}, {i}[1] AS _{i}".format(
                                    i=i[:-1]
                                )
                                for i in retain
                            ]
                        ...
                        + ", ".join(
                            [
                                i
                                if i[:-1] not in unwound
                                else "COLLECT([{i},_{i}]) AS {j}".format(
                                    i=i[:-1], j=i
                                )
                                for i in retain
                            ]
        ...
        if (
            strings["size"] == 1
            and "Depends" not in attack
            and "Cypher" not in attack
        ):
            ...
            cypher += " ".join(
                (
                    ...
                    "   AND target IN [_ IN options|_[0]] "
                    if (
                        "Depends" in attack
        ...
        cypher += " ".join(
            (
                ...
                if ("Grants" not in attack)
                else "WITH DISTINCT source, COLLECT([target, commands]) AS options, grants, admin ",
                ...
                "MERGE (source)-[edge:%s]->(pattern:Pattern:{name})"
                % str("CREATE" if options["CreateAction"] else "ATTACK"),
                "ON CREATE SET " '   edge.Name = "{name}", ',
                f"  edge.Transitive = {options['Transitive']}, "
    ...
    def compute(self, max_iterations=5):
        ...
        self.console.task(
            "Removing all existing attacks",
            db.run,
            args=[
                "MATCH (p) WHERE p:Pattern "
        ...
        for pattern, iteration, i in self.console.tasklist(
            ...
            attacks,
            done=lambda results: str(
                "Added {count} potential attack paths"
            ).format(
                count=sum(
                    [
                        len(s["results"]) if "results" in s else 0
                        for s in self.stats
                    ]
                )
                - pruned
            ...
            self.console.info(
                f"Searching for attack ({i:02}/{len(self.definitions):02}): "
            ...
            if i == len(self.definitions):  # End of iteration i
                ...
                if (
                    sum(
                        [
                            len(s["results"])
                            for s in self.stats[-(len(self.definitions)) :]
                        ]
                    )
                    == 0
                ):
                ...
                redundant = db.run(
                    "MATCH shortestPath((admin)-[r:ATTACK|TRANSITIVE*1..]->(:Admin)) "
                    "    WHERE NOT (admin:Pattern OR admin:Admin) "
                ...
                if redundant > 0:
                    self.console.debug(
                        f"{redundant} redundant admin paths "
