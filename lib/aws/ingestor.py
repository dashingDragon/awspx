...
from lib.aws.policy import BucketACL
from lib.aws.policy import IdentityBasedPolicy
...
class IngestionManager(Elements):
    ...
    def load_transitives(self):
        ...
        for resource in self.console.tasklist(
            ...
            if resource.label() in [
                ...
                if "AttachedManagedPolicies" in resource.properties():
                    ...
                    for policy in filter(
                        lambda r: r.id() in policy_arns, policies
                    ):
                        self.add(
                            Transitive(
                ...
                # (User)-->(Group)
                if (
                    resource.label() in ["AWS::Iam::User"]
                    and "GroupList" in resource.properties()
                ):
                    group_names = resource.get("GroupList")

                    for group in filter(
                        lambda r: r.get("Name") in group_names, groups
                    ):
                        self.add(
                            Transitive(
                                properties={"Name": "Attached"},
            ...
            if (
                resource.label() in ["AWS::Ec2::Instance"]
            ...
            if (
                resource.label() in ["AWS::Iam::InstanceProfile"]
                and "Roles" in resource.properties()
            ):
                role_arns = list(
                    map(lambda r: r["Arn"], resource.get("Roles"))
                )
            ...
            # (Function) --> (Role)
            if (
                resource.label() in ["AWS::Lambda::Function"]
        ...
        for resource in self.console.tasklist(
            ...
            if resource.label() in ["AWS::Iam::Role"]:
                ...
                for action in [
                    ...
                    if (
                        action.source().type("AWS::Account")
    ...
    def save(self, db="default", path="/opt/awspx/data"):
        ...
        directory = (
            f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}_{db.split('.')[0]}"
        )
        labels = sorted(
            list(
                {
        ...
        for label in self.console.tasklist(
            ...
            if (
                type(next(iter(elements))) is Node
                or Node in type(next(iter(elements))).__bases__
            ):
                prefix = [":ID"]
                suffix = [":LABEL"]
                data = [
                    [e.id()]
                    + [
                        stringify(e.get(f), _) if f in e.properties() else ""
                        for (f, _) in header
                    ]
                    + [";".join(e.labels())]
                    for e in elements
            ...
            if label == labels[-1]:
                shutil.make_archive(
                    f"{path}/{directory}", "zip", f"{path}/{directory}"
                )
...
class SessionClientWrapper:
    ...
    def __iter__(self):
        try:
            yield from self.client.__iter__()
...
class Ingestor(Elements):
    ...
    def __init__(
        ...
        if (
            ...
            not in self.session.get_available_services()
        ):
            self.console.critical(
                f"'{self.__class__.__name__}' is not a recognized boto service.\n"
        ...
        if (
            ...
            not in self.session.get_available_resources()
        ):
            self.console.critical(
                f"'{self.__class__.__name__}' is not a supported boto resource. "
        ...
        self.client = SessionClientWrapper(
            self.session.client(self.__class__.__name__.lower()),
            console=self.console,
        )
        ...
        if not load_resources and len(self.types) == 0:
            self.console.critical(
                f"No AWS::{self.__class__.__name__} resources were found in 'lib.aws.resources.py'. "
        ...
        self.types = [
            ...
            for r in map(
                lambda r: next(
                    (t for t in RESOURCES if t.upper() == r.upper()), None
                ),
                self.types,
            )
            if r is not None
    ...
    def load_resources(self):
        def get_resource_type_model(collection):
            ...
            model = {
                k: {
                    K: [] if "actions" in K.lower() else V
                    for K, V in v.items()
                }
                for k, v in {
                    **collection.meta.resource_model._resource_defs,
            ...
            attrs = {
                ...
                **reduce(
                    lambda y, x: {
                        **y,
                        x[0]: list(
                            {x[1], *list(y[x[0]] if x[0] in y else [])}
                        ),
                    },
                    [
                        (y["resource"]["type"], x)
            ...
            shapes = {
                k: model[k]["shape"] if "shape" in model[k] else None
                for k in model
            }

            loads = {
                k: model[k]["load"]["request"]["operation"]
                if "load" in model[k]
                else None
                for k in model
            }
            ...
            actions = reduce(
                lambda o, x: {
                    **o,
                    **{
                        k: list({*v, *list(o[k] if k in o else [])})
                        for k, v in x.items()
                    },
                },
                [
            ...
            def get_collections(key=service, collections=set()):
                for h, t in [
                    (h, model[key]["hasMany"][h]["resource"]["type"])
                    for h in list(
                        model[key]["hasMany"].keys()
                        if "hasMany" in model[key]
                        else []
                    )
                ]:
                    if any([r in collections for r in actions[operations[t]]]):
                        continue
            ...
            remap = {
                k: next(
                    filter(
                        lambda x: any(
                            [
                                f"AWS::{service}::{y}".lower() == x.lower()
                                for y in {k, shapes[k], *attrs[k]}
                            ]
                        ),
                        RESOURCES,
                    ),
            ...
            properties = [
                ...
                and p
                in [
                    model[c][h][k]["resource"]["type"]
                    for c in collections
                    for h in ["has", "hasMany"]
                    if h in model[c]
                ...
                and p
                not in [
                    v.split(":")[-1] for v in remap.values() if v is not None
                ]
            ...
            if None in remap.values():
                ...
                collections = {c for c in collections if c not in undefined}
            ...
            meta = {
                k: {
                    ...
                    # (collection, method)
                    "method": [
                        (x if x != service else None, r.name)
                        for x in [
                            service,
                            *[m for m in remap if remap[m] is not None],
                        ]
                        for r in boto3.resources.factory.ResourceModel(
                            x, model[x], model
                        ).collections
                        if r._definition["resource"]["type"] == k
                    ][0],
                    "loads": loads[k],
                    "props": {
                        t: loads[v["resource"]["type"]]
            ...
            meta = {
                v["label"]: {
                    x: y for x, y in v.items() if x not in ["method", "label"]
                }
            ...
            for attr, v in model.items():
                ...
                if len(self.types) > 0 and label not in self.types:
                    collateral = [
                        ...
                        if rt in self.types
                        and rt
                        not in [list(k.keys())[0] for k in model.values()]
                    ]
                ...
                for operation, collection in self.console.tasklist(
                    ...
                    for cm in SessionClientWrapper(
                        operation(), console=self.console
                    ):
                        if not (
                            "meta" in dir(cm) and "identifiers" in dir(cm.meta)
                        ):
                            self.console.warn(
                        ...
                        properties = dict(
                            cm.meta.data if cm.meta.data is not None else {}
                        )

                        if not all([k in properties for k in ["Arn", "Name"]]):
                            identifiers = {
                                        ...
                                        if "Arn" in identifiers
                                            ...
                                            **dict(
                                                ...
                                                if (
                                                    ...
                                                    and not collection.__class__.__name__.endswith(
                                                        "ServiceResource"
                                                    )
                            ...
                            if "Name" not in properties:
                                ...
                                if len(names) == 0:
                                    ...
                                    if match is None:
                                        self.console.warn(
                                            "Failed to construct resource Name from Arn: "
                                ...
                                if len(names) > 0:
                                    ...
                                    self.console.warn(
                                        "Failed to construct resource Name: "
                        ...
                        # Add Resource
                        resource = Resource(
                            labels=[label], properties=properties
                        )
                        self.add(resource)
    ...
    def load_associatives(self):
        ...
        for resource in self.console.tasklist(
            ...
            arn_refs = (
                {k: {matches.group(k)} for k in regex.groupindex.keys()}
            ...
            for rt in [
                [rt for rt in association if rt != resource.label()][0]
                ...
                if not all([len(v) == 1 for v in refs[0].values()]):
                    ambiguous = {
                        k: v for k, v in refs[0].items() if len(v) > 1
                    }
                    ...
                    refs = [
                        ...
                        for c in [
                            {k: v for d in list(x) for k, v in d.items()}
                ...
                for ref in refs:
                    ...
                    arn = RESOURCES.types[rt].format(
                        **{
                            k: str(list(v)[0] if type(v) == set else v)
                    ...
                    (source, target) = sorted(
                        (resource, associate), key=lambda r: r.id()
                    )
    ...
    def add(self, element):
        if any(r in element.labels() for r in ["Resource", "Generic"]):
            ...
            if "Resource" in element.labels() and (
                (
                    len(self._only_arns) > 0
                    and element.id() not in self._only_arns
                )
                or (
                    len(self._skip_arns) > 0
                    and element.id() in self._skip_arns
                )
            ):
                self.console.debug(
                    f"Skipping {element}: "
        ...
        if len(self) == length:
            ...
            [e in ["ASSOCIATIVE", "TRANSITIVE"] for e in element.labels()]
        ):
            self.console.info(
                f"Added {element.label().capitalize()} relationship: "
                f"({element.source()}) → ({element.target()})"
    ...
    def destroy(self):
        ...
        self.console.notice(
            f"Added {resources} Resource(s), {generics} Generic(s), "
...
class IAM(Ingestor):
    ...
    associations = [("AWS::Iam::User", "AWS::Iam::VirtualMfaDevice")]
    ...
    def get_account_authorization_details(self):
            ...
            for k in sorted(item.keys()):
                ...
                if k.endswith("PolicyList"):
                    properties["Documents"] = [
                        {p["PolicyName"]: p["PolicyDocument"] for p in item[k]}
                    ]
                ...
                # Rename PolicyVersionList to Document
                    ...
                    properties["Document"] = [
                        {
                            "DefaultVersion": [
                                p for p in item[k] if p["IsDefaultVersion"]
                            ][0]["Document"]
                        }
                    ]
        ...
        for page in self.console.tasklist(
            ...
            account_authorization_details = [
                (
                    k.replace("DetailList", "").replace("Policies", "Policy"),
                    detail,
                )
    ...
    def list_user_mfa_devices(self):
        ...
        for user in self.console.tasklist(
            ...
            for mfa_device in self.client.list_mfa_devices(
                ...
                mfa_device["Name"] = (
                    mfa_device["Arn"].split("/")[-1]
                    if label == "AWS::Iam::MfaDevice"
                    else "Virtual Device"
                    if label == "AWS::Iam::VirtualMfaDevice"
                    else "Device"
                )
...
class EC2(Ingestor):
    ...
    def load_associatives(self):
        ...
        self.difference_update(
            set(
                filter(
                    lambda a: isinstance(a, Associative)
                    and (
                        a.source().type("AWS::Ec2::RouteTable")
                        and a.target().type("AWS::Ec2::Vpc")
                    )
                    and any(
                        [
                            "Main" in r and not r["Main"]
                            for r in list(
                                a.source().properties()["Associations"]
    ...
    def get_nat_gateways(self):
        ...
        for nat_gateway in [
            ng
            for r in self.console.tasklist(
                "Adding NatGateways",
                iterables=self.client.get_paginator(
            ...
            nat_gateway["Arn"] = RESOURCES.definition(
                "AWS::Ec2::NatGateway"
            ).format(
                Region=self.session.region_name,
                Account=self.account,
            ...
            nat_gateway = Resource(
                properties=nat_gateway, labels=["AWS::Ec2::NatGateway"]
            )
        ...
        for instance in self.console.tasklist(
            ...
            try:
                response = self.client.describe_instance_attribute(
                    Attribute="userData", DryRun=False, InstanceId=name
                )
            except ClientError:
                self.console.error(
                    f"Couldn't get user data for {name} "
                    "- it may no longer exist."
                )
                continue

            if (
                "UserData" in response.keys()
                and "Value" in response["UserData"].keys()
            ):
                userdata = b64decode(response["UserData"]["Value"])
                if userdata[0:2] == b"\x1f\x8b":  # it's gzip data
                    userdata = zlib.decompress(
                        userdata, zlib.MAX_WBITS | 32
                    ).decode("utf-8")
                else:  # normal b64
...
class S3(Ingestor):
    ...
    associations = [("AWS::S3::Bucket", "AWS::S3::Object")]
...
class Lambda(Ingestor):
    ...
    def list_functions(self):
        ...
        for function in [
            ...
            function = Resource(
                properties=function, labels=["AWS::Lambda::Function"]
            )
