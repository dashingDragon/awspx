...
class Statement:
    ...
    def __init__(self, statement, resource, resources):
        ...
        if not (
            ...
            console.critical(
                f"Statement: {self.__statement} " "is missing required key"
            )

        if "NotPrincipal" in keys:
            console.warn(
                "'NotPrincipal' support hasn't been implemented."
        ...
        if (
            not any([k in keys for k in ["Resource", "NotResource"]])
    ...
    def _get_principals(self):
        ...
        key = list(
            filter(lambda x: "Principal" in x, self.__statement.keys())
        )[0]
        ...
        if "AWS" in statement:
            ...
            if "*" in statement["AWS"]:
                ...
                principals = Elements(
                    [
                        *self.__resources.get("AWS::Iam::User").get(
                            "Resource"
                        ),
                        *self.__resources.get("AWS::Iam::Role").get(
                            "Resource"
                        ),
                        external,
                    ]
            ...
            for principal in [
                ...
                node = next(
                    (
                        a
                        for a in self.__resources.get("Resource")
                        if a.id() == principal
                    ),
                    None,
                )
                ...
                if node is None:
                    ...
                    if (
                        re.compile(f"^{RESOURCES.regex['Account']}$").match(
                            principal
                        )
                        is not None
                    ):
            ...
            services = (
                statement["Service"]
                if isinstance(statement["Service"], list)
                else [statement["Service"]]
            )
            ...
            for service in services:
                ...
                principals.add(
                    Node(
                        key="Name", labels=labels, properties={"Name": service}
                    )
                )
            ...
            for federated in statements:
                if (
                    re.compile(RESOURCES["AWS::Iam::SamlProvider"]).match(
                        federated
                    )
                    is not None
                ):
                    base = (
                        ...
                        if (
                            next(
                                (
                                    a
                                    for a in self.__resources.get("Resoure")
                    ...
                    re.compile(RESOURCES["AWS::Iam::OidcProvider"]).match(
                        federated
                    )
                    is not None
                ):
    ...
    def _get_resources_and_conditions(self):
        ...
        key = list(filter(lambda x: "Resource" in x, self.__statement.keys()))[
            0
        ]
        ...
        for rlp in {
            ...
            variables = list(re.findall(r"\$\{[0-9a-zA-Z:]+\}", rlp))
            regex = re.compile(
            ...
            for result in [
                ...
                condition = {
                    "StringEquals": {
                        variables[i]: regex.match(result.id()).group(
                            offset + i
                        )
    ...
    def actions(self):
        ...
        for action in self._get_actions():
            ...
            for affected_type in ACTIONS[action]["Affects"].keys():
                ...
                if affected_type == "AWS::Iam::Policy" and ACTIONS[action][
                    "Access"
                ] in ["Permissions Management", "Write"]:
                    action_resources.update(
                        [
                            a
                            for a in resources.get(affected_type)
            ...
            for resource in action_resources:
                ...
                resource_conditions = list(
                    ...
                    if str(resource) in conditions
                    else [{}]
                )

                statement_conditions = dict(
                    self.__statement["Condition"]
                    if "Condition" in self.__statement.keys()
                    else {}
                )
                ...
                condition = (
                    json.dumps(
                        [
                            {**resource_conditions[i], **statement_conditions}
                            for i in range(len(resource_conditions))
                        ]
                    )
                    if (
                        len(resource_conditions[0]) + len(statement_conditions)
                    )
                    > 0
                    else "[]"
                )
    ...
    def resources(self):
        if self._resources is None:
            (
                self._resources,
                self._conditions,
            ) = self._get_resources_and_conditions()
    ...
    def __init__(self, document, resource, resources):
        ...
        for statement in self.document["Statement"]:
            self.statements.append(
                Statement(
                    statement=statement, resource=resource, resources=resources
                )
            )
...
class Policy:
    ...
    def actions(self):
        ...
        console.info(
            f"{self.__class__.__name__} {self.__resource} "
...
class IdentityBasedPolicy(Policy):
    def __init__(self, resource, resources):
        ...
        key = list(
            filter(
                lambda k: k == "Document" or k == "Documents",
    ...
    def __init__(self, resource, resources):
        ...
        for key, acls in resource.properties().items():
            ...
            if not (
                ...
                and "Grants" in acls
                and all(
                    ["Grantee" in x and "Permission" for x in acls["Grants"]]
                )
            ):
                continue

            # Construct a policy from ACL
            for grantee, permission in map(
                lambda x: (x["Grantee"], x["Permission"]), acls["Grants"]
            ):
