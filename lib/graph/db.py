...
class Neo4j:
    ...
    databases = [
        db
        for db in os.listdir(f"{NEO4J_DB_DIR}/")
    ...
    def _switch_database(self, db):
        ...
        subprocess.Popen(
            [
                "sed",
                "-i",
                r"/^\(#\)\{0,1\}dbms.default_database=/s/.*/dbms.default_database=%s/"  # noqa: E501
                % db,
    ...
    def _load(self, archives, db):
        ...
        for archive in archives:
            ...
            ARCHIVES[archive]["CSV"] = [
                f
                for f in os.listdir(ARCHIVES[archive]["DIR"])
                if f.endswith(".csv")
            ]
            ...
            for i in range(2):
                for _, v in ARCHIVES.items():
                    if c not in v["CSV"]:
                        ...
                        with open(f'{v["DIR"]}/{c}') as f:
                            headers = [
                                h.strip() for h in f.readline().split(",")
                            ]
        ...
        csvs = [
            f"{a['DIR']}/{csv}" for a in ARCHIVES.values() for csv in a["CSV"]
        ]

        edges = [
            e for e in csvs if re.compile(r"(.*/)?([A-Z]+)\.csv").match(e)
        ]
        ...
        return str(
            f"Loaded {nodes} nodes, {edges} edges, and {props} properties "
            f"into '{db}' from: {', '.join([re.sub(f'^{NEO4J_ZIP_DIR}/', '', a) for a in archives])}"  # noqa: E501
        )
    ...
    def load_zips(self, archives=[], db="neo4j"):
        ...
        self.console.task(
            f"Switching active database to '{db}'",
            self._switch_database,
            args=[db],
